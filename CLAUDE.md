...
- `tcitester.py` - Exercises a TCI server's SET commands one at a time and checks the radio agreed. Each test is a round trip: read the current value, send a new one, wait for the server to BROADCAST the change (only a broadcast proves the command reached the radio - a reply does not, since the server answers with the value it currently holds either way), then restore the original. `--list`, `--dry-run`, `--safe` (receive-side settings only, the default), `--all` (adds transmit power), `--only <name>`. Never keys the transmitter. Kept separate from `tcimonitor.py` on purpose: the monitor is read-only and therefore safe to leave running against a live rig, and merging write capability into it would destroy that property.
